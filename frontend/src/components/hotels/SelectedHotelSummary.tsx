      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-0.5">
          ...
          {stay.booking?.confirmation_ref && (
            <p className="text-xs font-body text-ink-muted">
              Confirmation: <span className="font-mono text-ink">{stay.booking.confirmation_ref}</span>
            </p>
          )}
        </div>
        <button
          onClick={onRemove}
          className="shrink-0 text-xs px-2.5 py-1 rounded-lg font-body transition-colors text-ink-muted border border-border hover:border-red/40 hover:text-red"
        >
          Remove
        </button>
      </div>
